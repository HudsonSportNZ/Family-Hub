...
const MEMBER_STYLES: Record<Member, { color: string; bg: string; label: string }> = {
  M: { color: '#6C8EFF', bg: 'rgba(108,142,255,0.12)', label: 'Mum' },
  D: { color: '#34D399', bg: 'rgba(52,211,153,0.12)',  label: 'Dad' },
  I: { color: '#F472B6', bg: 'rgba(244,114,182,0.12)', label: 'Isabel' },
  J: { color: '#FBBF24', bg: 'rgba(251,191,36,0.12)',  label: 'James' },
}
    ...
    <>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ph-action { font-size: 10px; color: var(--muted); cursor: pointer; padding: 3px 9px; border-radius: 7px; background: var(--card2); border: 1px solid var(--border); transition: all 0.2s; font-weight: 500; text-decoration: none; display: inline-block; }
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          <div className="grid">
              ...
              <div className="ph">
                <div className="ph-left">
                  ...
                  <div>
                    ...
                    <div className="ph-sub">{new Date().toLocaleDateString('en-NZ', { weekday:'short', day:'numeric', month:'short' })} · 6 things happening</div>
                  </div>
                </div>
            ...
            {/* TODAY'S TASKS — live from Supabase */}
            <div className="panel">
              <div className="ph">
                <div className="ph-left">
              ...
              <div className="pb">
                {todayTasks.length === 0 ? (
                  <div className="empty-tasks">
                    <span>✨</span>
                    <p>No tasks today</p>
                    <a href="/tasks" style={{fontSize:10, color:'var(--accent)', textDecoration:'none'}}>+ Add tasks</a>
                  </div>
                ) : (
                  todayTasks.slice(0, 6).map(task => {
                    const done = isCompleted(task.id)
                    const primaryMember = task.assigned_to[0] as Member | undefined
                    const style = primaryMember ? MEMBER_STYLES[primaryMember] : { color: '#94A3B8', bg: 'rgba(148,163,184,0.12)', label: '–' }
                    return (
                      <div key={task.id} className="task-row" onClick={() => toggleTask(task)}>
                        ...
                        {primaryMember && (
                          <div className="task-tag" style={{background: style.bg, color: style.color}}>
                            {style.label}
                          </div>
                        )}
                      </div>
                ...
                {todayTasks.length > 6 && (
                  <a href="/tasks" style={{fontSize:10, color:'var(--muted)', textDecoration:'none', textAlign:'center', paddingTop:4}}>
